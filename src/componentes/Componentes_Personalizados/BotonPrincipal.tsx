...
export function Boton({ accion, type, mensaje, icono, colores, title, name }: MensajeBoton) {
    const colores_base = 'bg-[#1E2939] hover:bg-[#374151] text-white';

    return (
        <button type={type}  onClick={accion} title={title} name={name} className={`flex flex-wrap gap-1 items-center cursor-pointer p-1 rounded-sm ${colores ? colores : colores_base}`}>
